...
#pragma mark add region
- (void)getCurrentLocation{
    __weak typeof(self) weakself = self;
    [self.locationManager requestLocationWithReGeocode:NO completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        [weakself addCircleRegionForCoordinate:location.coordinate];
    }];
}

- (void)addCircleRegionForCoordinate:(CLLocationCoordinate2D)coordinate{
    //创建圆形围栏
    AMapLocationCircleRegion *cirRegion200 = [[AMapLocationCircleRegion alloc]initWithCenter:coordinate radius:200.0 identifier:@"circleRegion200"];
    AMapLocationCircleRegion *cirRegion300 = [[AMapLocationCircleRegion alloc]initWithCenter:coordinate radius:300.0 identifier:@"circleRegion300"];
    //添加地理围栏
